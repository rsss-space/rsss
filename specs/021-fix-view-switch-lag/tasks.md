...
- [X] T001 Run `npm test && npm run lint && npm run typecheck` from the
...
- [X] T002 Add the `FilterKey`, `ViewItemsCacheEntry`, and
...
- [X] T003 Add `viewItemsCache:ViewItemsCache` to the `AppState`
...
- [X] T004 Add a `currentFilterKey(state:AppState):FilterKey | null`
...
- [X] T005 Add the `applyItemsResult(state, requestKey, result)`
...
- [X] T006 Rewrite `State.loadItems` in `src/client/state.ts` to
...
- [X] T007 Gate the `itemsLoading=true` write inside
...
- [X] T008 [P] [US1] Create `test/view-switch-instant.ts` that
...
- [X] T009 [P] [US1] Extend `test/sidebar-item.ts` to mount the
...
- [X] T010 [US1] Rewrite `State.showAll` and `State.showStarred` in
...
- [X] T011 [US1] Rewrite `src/client/components/sidebar-item.ts` so
...
- [X] T012 [US1] Verify the route-event integration: when the user
...
- [X] T013 [P] [US2] Create `test/view-switch-stale-refresh.ts`.
...
- [X] T014 [US2] Verify, by reading the call sites, that every
...
- [X] T015 [US2] Confirm in-place update behavior (FR-005, SC-003):
...
- [X] T016 [P] [US3] Create `test/view-switch-cache-invalidate.ts`.
...
- [X] T017 [US3] In `src/client/state.ts`, add
...
- [X] T018 [US3] In the same file, add
...
- [X] T019 [US3] Verify pull-sync invalidation: open
...
- [X] T020 [US3] Confirm the `itemsLoading` gate from T007 in
...
- [X] T022 [P] Run `npm test` and confirm the three new specs
...
- [X] T023 [P] Run `npm run lint` and `npm run typecheck`. Fix any
      `state.ts` line-length violations introduced by the
      `applyItemsResult` rewrite (≤ 80 cols per global CLAUDE.md).

- [X] T024 [P] Read the final `src/client/state.ts` and
